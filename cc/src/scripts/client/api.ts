...
export class ClientAPI {
  ...
  private p2pRequestHandler?: (this: void, src: string, action: string, params: Record<string, unknown>) => Promise<unknown> | unknown;
  ...
  public onP2PRequest(
    handler: (this: void, src: string, action: string, params: Record<string, unknown>) => Promise<unknown> | unknown
  ): void {
    this.p2pRequestHandler = handler;
  }

  /** Registers a handler function for incoming direct P2P data messages */
  public onP2PMessage(handler: (this: void, src: string, payload: DirectMessagePayload) => void): void {
    ...
    while (true) {
      ...
      if (src === this.serverId) {
        ...
        if (category === PacketCategory.SIGNAL) {
          ...
          try {
            const handshake = textutils.unserialiseJSON(packet.payload.ciphertext) as P2PHandshakePayload;
            if (handshake && handshake.ticket && handshake.iv) {
              ...
              if (p2pTicket && p2pTicket.sessionKey && p2pTicket.initiatorId) {
                this.sessionManager.setSessionKey(p2pTicket.initiatorId, p2pTicket.sessionKey, p2pTicket.ttl);
              }
            }
          } catch {
              ...
              Promise.resolve(this.p2pRequestHandler(src, reqData.action, reqData.params || {}))
                .then((result) => {
                  const resPacket = this.protocol.createResponsePacket(
                    src, replyToNonce, true, result, undefined, sessionKey
                  );
                  this.network.sendPacket(resPacket);
                })
                .catch((err) => {
                  const resPacket = this.protocol.createResponsePacket(
                    src, replyToNonce, false, undefined, err instanceof Error ? err.message : String(err), sessionKey
                  );
                  this.network.sendPacket(resPacket);
                });
            }
          ...
          const sessionKey = this.sessionManager.getSessionKey(src);
          if (sessionKey) {
            const messageData = this.protocol.unpackPayload<DirectMessagePayload>(packet, sessionKey);
   ...
   */
  public async requestP2P(peerId: string, action: string, params: Record<string, unknown> = {}): Promise<unknown> {
  ...
  public async sendP2P(peerId: string, content: string | DirectMessagePayload): Promise<void> {
    ...
    const payload: DirectMessagePayload = typeof content === "string"
