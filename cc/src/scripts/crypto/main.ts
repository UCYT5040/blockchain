...
export function speckCTR(dataBytes: number[], keyHex: string, ivHex: string): number[] {
  ...
  for (let i = 0; i < dataBytes.length; i += 8) {
    ...
    const keystream = [
      ksHi & 0xff, (ksHi >> 8) & 0xff, (ksHi >> 16) & 0xff, (ksHi >> 24) & 0xff,
      ksLo & 0xff, (ksLo >> 8) & 0xff, (ksLo >> 16) & 0xff, (ksLo >> 24) & 0xff
    ];
...
export function blake2s(inputStrOrBytes: string | number[], keyHex?: string): string {
  const bytes = typeof inputStrOrBytes === "string" ? stringToBytes(inputStrOrBytes) : inputStrOrBytes;
