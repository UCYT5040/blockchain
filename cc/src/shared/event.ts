...
export class PeripheralEvent implements IEvent {
	...
	public static init(args: any[]): IEvent | null {
		if (
			!(typeof args[0] === 'string') ||
			((args[0] as string) != 'peripheral' && (args[0] as string) != 'peripheral_detach')
		)
			return null;
...
export class ModemMessageEvent implements IEvent {
	...
	public get_args() {
		return [this.side, this.channel, this.replyChannel, this.message, this.distance];
	}
	public static init(args: any[]): IEvent | null {
...
export class HTTPEvent implements IEvent {
	...
	public static init(args: any[]): IEvent | null {
		if (
			!(typeof args[0] === 'string') ||
			((args[0] as string) != 'http_success' && (args[0] as string) != 'http_failure')
		)
			return null;
...
export class WebSocketEvent implements IEvent {
	...
	public static init(args: any[]): IEvent | null {
		if (
			!(typeof args[0] === 'string') ||
			((args[0] as string) != 'websocket_success' && (args[0] as string) != 'websocket_failure')
		)
			return null;
...
export class MouseEvent implements IEvent {
	...
	public get_args() {
		return [this.type == MouseEventType.Touch ? this.side : this.button, this.x, this.y];
	}
	public static init(args: any[]): IEvent | null {
...
export class ResizeEvent implements IEvent {
	...
	public static init(args: any[]): IEvent | null {
		if (
			!(typeof args[0] === 'string') ||
			((args[0] as string) != 'term_resize' && (args[0] as string) != 'monitor_resize')
		)
			return null;
